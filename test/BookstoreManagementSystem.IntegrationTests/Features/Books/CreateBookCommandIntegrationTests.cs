...
//
//         [Fact]
//         public async Task CreateBook_WithNonExistingGenre_ShouldThrowException()
//         {
//             // Arrange
//             var authors = await _dbContext.Authors.ToListAsync();
//             var authorIds = authors.Select(a => a.Id).ToArray();
...
//                 AuthorList = authorIds,
//                 GenreList = new short[] { 99 } // Non-existing genre
//             });
//
//             // Act & Assert
//             var exception = await Assert.ThrowsAsync<Exception>(() => 
//                 _handler.Handle(command, CancellationToken.None));
//             
//             Assert.Equal("Genre with ID=99 not found", exception.Message);
//         }
//
//         [Fact]
//         public async Task CreateBook_ShouldSetCreatedAtAndUpdatedAtTimestamps()
//         {
//             // Arrange
//             var authors = await _dbContext.Authors.ToListAsync();
//             var authorIds = authors.Select(a => a.Id).ToArray();
...
//             
//             Assert.NotNull(bookInDb);
//             Assert.True(bookInDb.CreatedAt >= testStartTime);
//             Assert.True(bookInDb.UpdatedAt >= testStartTime);
//             Assert.Equal(bookInDb.CreatedAt, bookInDb.UpdatedAt);
//         }
//
//         [Fact]
//         public async Task CreateBook_WithMultipleAuthorsAndGenres_ShouldCreateAllRelationships()
//         {
//             // Arrange
//             var authors = await _dbContext.Authors.ToListAsync();
//             var authorIds = authors.Select(a => a.Id).ToArray();
...
//             {
//                 Price = 39.99m,
//                 Title = "Book with multiple relationships",
//                 AuthorList = authorIds,
//                 GenreList = new short[] { 1, 2, 3 } // All genres
//             });
//
//             // Act
//             var result = await _handler.Handle(command, CancellationToken.None);
...
//             
//             Assert.NotNull(bookInDb);
//             Assert.Equal(authorIds.Length, bookInDb.BookAuthors.Count);
//             Assert.Equal(3, bookInDb.BookGenres.Count);
//         }
//     }
// }
